...
**23. Undo and redo are keyboard primary, scoped to editor focus, with native textfield precedence inside inputs.** `Ctrl+Z` pops the most recent entry, restores `mapping_before` via `SetMapping`, and pushes the popped entry to a redo stack. `Ctrl+Shift+Z` reverses. Both shortcuts capture only when focus is inside the editor; outside the editor the shortcuts are unbound for now. **Inside a focused `<input>`** (name field, numeric stage-body field, KeyCombo capture, etc.), `Ctrl+Z` falls through to the browser's native textfield undo (keystroke-level). Once the field blurs (committing the edit to engine + undo log), `Ctrl+Z` drives the editor's per-mapping undo stack. This matches OS conventions and aligns with choice 29's commit-on-blur dispatch model. The footer recap text reads the last entry from the active mapping's stack (`Undo: deadzone outer 92% → 95%` shows the inverse of what would happen if Ctrl+Z fires next).
...
│   │   ├── undo_log.rs                          # UndoStore + Ctrl+Z / Ctrl+Shift+Z handlers
│   │   ├── pipeline/
...
15. Per-mapping session-undo stack records every commit. `Ctrl+Z` and `Ctrl+Shift+Z` capture only when focus is inside the editor. Stack is cleared on profile flip via the existing F4 dirty-confirm dialog.
...
21. Tab order matches DOM order. Stage chevron Space and Enter both toggle expand. Esc on a focused stage is a no-op when capture is not armed. Ctrl+Z and Ctrl+Shift+Z drive the editor undo log only when focus is outside text inputs; inside an input, Ctrl+Z falls through to native field undo until blur.
