...
**1. Editor sections in fixed vertical order.** Header (title with inline rename plus subtitle with source label + rebind + optional output label), Live readout, Inactive-runtime hint (when divergent), Pipeline graph, Undo recap footer. All sections are 1 px divider separated; consistency over rhythm because the product register favors predictable structure. (**Amended 2026-05-01.** The original section list split Header, Name field, and Input field across three consecutive rows. The amendment collapses them under one Header block per DESIGN.md §6 "Inline-First Rule": "If a property has a single editable value, it is edited where it is displayed. The dialog opening on click is the canonical Tk failure; the system rejects it as a default." The h2 was a display surface, the name input was an editor for the same property, and the input field carried the source label that already lived in the subtitle: three rows for two properties. The merged Header carries each surface once.)

**2. Header is `<h2>` with inline rename plus subtitle row carrying source, rebind, and output.** Mapping name renders as a real `<h2>` (the chrome top bar is the page-level `<h1>`). The h2 has `tabindex="0"` and the F8 `data-editor-focus` marker so keyboard nav from the rail lands here. `Key::F2` and right-click on the focused h2 swap it for an inline `<input type="text">` styled to inherit h2 typography (20 px Inter weight 600, line-height 28 px), so the swap is visually invisible. Enter / blur commits via `EngineCommand::SetMapping`; Esc reverts. Empty / whitespace-only inputs revert without dispatch. The undo entry is pushed only when dispatch succeeds (no phantom entries while the engine is offline). Subtitle row reads `<source-label> [rebind] →   <output-label>` in JetBrains Mono 12 px, with the rebind affordance anchored next to the source label as a small ghost `Button`. The `rebind` button arms F8's live-capture primitive in `Any` filter mode; captured input replaces the mapping's `InputAddress` via a fresh `SetMapping` dispatch with the same actions and name, and pushes a `Rebind` undo entry on success. Cross-mode collision uses F8's redirect UX; Esc cancels the live-capture per F8's primitive contract. Source and output truncation: wrap to two lines when overflow, with the arrow leading the second line. Sim users need full identifiers; do not truncate device names with ellipsis. The h2 carries an F2 `Tooltip` on hover that shows the full current name when truncated.

**3. Subtitle output tail is omitted when no MapToVJoy stage exists.** Engine permits keyboard-only or mode-change-only mappings (F8 contract). When no `Action::MapToVJoy` is present in the action tree, the subtitle reads `<source-label> [rebind]` only, no arrow, no output. Live-readout OUT row is also hidden in this state.

**4. Superseded by choice 2 (amended 2026-05-01).** Originally specified a separate Name field row below the h2 (`width: 100%; max-width: 480px;`, horizontal-scroll for long names). The Inline-First Rule application folds rename into the h2 itself; the inline editor inherits h2 typography so long names still scroll horizontally inside the input.

**5. Superseded by choice 2 (amended 2026-05-01).** Originally specified a separate Input field row below the Name field carrying the source label and a `rebind` ghost button. The source label already appeared in the header subtitle; the duplicated row was a structural Inline-First violation. The amendment anchors the rebind button next to the subtitle's source label, retaining the Button ghost variant and the F8 `LiveCapture::Any` arming behavior.
...
**24. Tab order matches DOM order, no `tabindex` hacks.** Header h2 (display mode; `tabindex="0"`, F8 `data-editor-focus` target; F2 / right-click arms inline rename), subtitle rebind button, first stage header chevron, body fields when expanded, next stage header, add-stage button, undo recap (skip; non-interactive). DOM order matches visual order so default tab works. (**Amended 2026-05-01** to drop the separate Name-field input from the tab list per choice 1's amendment; the h2 is now the rename entry point and inherits the F8 focus marker.)
