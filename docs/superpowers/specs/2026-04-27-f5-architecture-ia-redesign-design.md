...
16. **Side-panel discipline: Replace.** Devices and Profiles are right-side panels sharing one slot; opening one closes the other. Top bar shows which is open via the active-tab style. Stacked panels and tabbed panels both rejected.
...
- **Mode tabs** — editing-scope only. Active editing tab gets focus-cyan underline. Engine's runtime mode shows as small dot on whichever tab the engine is in (green = natural, amber = forced override). "+" creates a new mode inline. Right-click menu: Activate · Rename · Delete · Set as default.
...
Status bar text never raises to bright; it is glance-only. Click targets are the top bar and the side panels, not the status bar. The status bar exists to fill peripheral vision during tuning sessions, not to be acted on.
...
```rust
pub struct Snapshot {
    pub id:           SnapshotId,            // ULID or timestamp-based
...
- Mode tabs render editing scope; active tab gets focus-cyan underline; runtime marker dot is computed from `MetaSnapshot.current_mode`. Marker color is green when natural, amber when forced (forced state surfaced from a new field on `MetaSnapshot` introduced by F7 itself or by F6 if scoped there).
- Mode tab right-click menu uses F2 Menu primitive; items: Activate · Rename · Delete · Set as default. Active mode's Activate item is disabled.
...
**Owns:** the mapping-list component, filter input, group headers, row component (name + source + glyph annotations), `+ Add mapping` expanding row, empty state, mapping-list-level keyboard navigation (Up/Down to move selection, Enter to focus editor, Cmd-F to focus filter).
...
**Blocks:** F9 (editor renders from the rail's selection).
...
- Symmetric handles or independent handles per side (decide during shape — today's egui behavior is the floor).
- Keyboard reachable.
...
**Blocks:** none (parallel with F13 except for the panel-slot replace coordination).
...
**Depends on:** F7 chrome (panel toggle, profile-name click target), F4 dialog (multiple destructive ops), F6 (snapshot module + engine commands).
**Blocks:** **default feature flag flips to `gui-dioxus` when this merges**, per the migration strategy. All core and secondary surfaces are now in place.

**Key requirements:**
- Replace discipline with F12.
...
- `+ New` expanding row: name + template radio (Blank / Copy from active / Copy from selected); active when at least one library row is selected.
...
### F15 — UX polish & audit (renamed; structurally unchanged)
...
| F15 UX polish | F15 unchanged | unchanged |
| F16 Cutover | F16 unchanged | unchanged |

**Counts:** parent plan had F1–F16 (16 features, 4 done, 1 in progress, 11 provisional). New plan: F1–F16 (16 features, 4 done, this spec is F5, 11 onwards rewritten). Same total count; very different scope distribution.
...
- **Live-input picker for `+ Add mapping` and `change input` flows.** A capture mode must own input events globally for a brief window. Implementation detail; F8 owns it.
...
- **Testing story for the new IA.** The parent plan's open question on Dioxus testing remains. F15 / F16 commits an approach.
...
2. Update the parent master plan (`2026-04-24-egui-to-dioxus-rewrite-design.md`) to incorporate the F6–F16 rewrite. Replace the "Core Screens — *provisional pending F5*" section with the new feature list. Add a "Resolved by F5" status note where relevant.
