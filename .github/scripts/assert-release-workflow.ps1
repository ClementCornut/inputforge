...
Assert-WorkflowContains -Text 'cargo install cargo-binstall --locked --force' -Reason "cargo-binstall install refreshes stale cached binaries"
Assert-WorkflowContains -Text 'cargo binstall --force --no-confirm --locked "dioxus-cli@${{ env.DIOXUS_CLI_VERSION }}"' -Reason "Dioxus CLI install refreshes stale cached binaries"
Assert-WorkflowContains -Text 'cargo install dioxus-cli --version "${{ env.DIOXUS_CLI_VERSION }}" --locked --force' -Reason "Dioxus CLI fallback install refreshes stale cached binaries"
Assert-WorkflowContains -Text "Verify Dioxus CLI" -Reason "workflow verifies the Dioxus CLI executable before bundling"
Assert-WorkflowContains -Text '$cargoHome = if ($env:CARGO_HOME) { $env:CARGO_HOME } else { Join-Path $env:USERPROFILE ".cargo" }' -Reason "Dioxus CLI verification handles hosted runners without CARGO_HOME"
Assert-WorkflowContains -Text 'Join-Path $cargoHome "bin\dx.exe"' -Reason "Dioxus CLI verification uses the expected Cargo-installed executable path"
Assert-WorkflowContains -Text 'Test-Path -LiteralPath $dx' -Reason "Dioxus CLI verification fails fast when the executable is missing"
Assert-WorkflowContains -Text '& $dx --version' -Reason "Dioxus CLI verification executes the installed binary"
Assert-WorkflowContains -Text 'DX_BIN=$dx' -Reason "workflow exports the verified Dioxus executable path"
Assert-WorkflowContains -Text '& $env:DX_BIN bundle --package inputforge-app --bin inputforge' -Reason "bundle step uses the verified Dioxus executable and lowercase binary target"
Assert-WorkflowContains -Text "Assert Windows installer source contract" -Reason "workflow statically verifies the Windows installer source contract"
Assert-WorkflowContains -Text "Verify SDL runtime source" -Reason "workflow fails fast when the Windows SDL runtime is missing"
Assert-WorkflowContains -Text 'Test-Path -LiteralPath "SDL/SDL3.dll"' -Reason "SDL runtime check uses the expected repo-local DLL path"
Assert-WorkflowContains -Text "Assert Windows bundle payload" -Reason "workflow verifies the generated Windows bundle payload"
Assert-WorkflowContains -Text '-BundleRoot "target/dx/inputforge/bundle/windows/nsis"' -Reason "bundle payload assertion targets the lowercase Dioxus output directory"
...
Assert-WorkflowContains -Text '$targetName = "inputforge_{0}_x64-setup.exe" -f $env:RELEASE_VERSION' -Reason "final installer basename uses exact lowercase product name and release version"
Assert-WorkflowDoesNotContain -Text '$targetName = $installer.Name.Replace($env:NSIS_BUNDLE_VERSION, $env:RELEASE_VERSION)' -Reason "final installer basename does not inherit Dioxus product casing"
...
Assert-StepBefore -FirstName "Install Dioxus CLI" -SecondName "Verify Dioxus CLI" -Reason "Dioxus CLI is installed before its executable path is verified"
Assert-StepBefore -FirstName "Verify Dioxus CLI" -SecondName "Bundle desktop app" -Reason "bundling uses a verified Dioxus CLI executable"
Assert-StepBefore -FirstName "Assert Windows installer source contract" -SecondName "Bundle desktop app" -Reason "installer source contract is checked before bundling"
Assert-StepBefore -FirstName "Verify SDL runtime source" -SecondName "Bundle desktop app" -Reason "SDL runtime source exists before bundling"
Assert-StepBefore -FirstName "Bundle desktop app" -SecondName "Assert Windows bundle payload" -Reason "bundle payload exists before it is checked"
Assert-StepBefore -FirstName "Assert Windows bundle payload" -SecondName "Locate installer" -Reason "installer discovery runs after payload verification"
